<?php include('include/header.php'); ?>
   ...
   <?php include('include/sidebar.php'); ?>
      ...
      <div class="content-wrapper">
        ...
        <?php
               $sql = "SELECT * FROM  tbl_contact WHERE flag = '0'";
           ...
           <?php
               $sql = "SELECT * FROM  tbl_contact WHERE flag = '1'";
               $message = $db->select($sql);
               if ($message) {
                $j = 0;
                  while ( $result = $message->fetch_assoc()) {
           ...
           ?>
                     ...
                     <span class="label label-waring pull-right"><?php if (isset($j)) {
                      echo $j;
                      } else echo "0"; ?>
             </a></li>
               </span>
       ...
       <?php 
            if(isset($_GET['textid'])){
              $textid = $_GET['textid'];
              $delQuery = "DELETE FROM tbl_contact WHERE id='$textid'";
    ...
    <?php
               $sql = "SELECT * FROM  tbl_contact WHERE flag = '1' ORDER BY id DESC";
               $message = $db->select($sql);
               if ($message) {
                $i = 0;
                  while ( $result = $message->fetch_assoc()) {
     ...
     ?>
                  ...
                  <tbody>
                     <tr>
                          ...
                          <td><?php echo  $sql_injection->textShorten($result['message'],20);?></td>
