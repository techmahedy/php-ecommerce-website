	...
	<h2 class="title text-center">Features Items</h2>
          <?php
           if (isset($_GET["brand_id"])){
            $brand_id = preg_replace('/[^-a-zA-Z0-9_]/', '',$_GET['brand_id']);
              }else{
              	echo "<script> window.location = '404'; </script>";
              }       

           $query = "SELECT * FROM tbl_product where brand_id='$brand_id' and type='0'";
...
<div class="features_items"><!--features_items-->
	<h2 class="title text-center">Non Features</h2>
     ...
     <?php
           ...
           $query = "SELECT * FROM tbl_product where brand_id='$brand_id' and type='1'";
           $result = $db->select($query);
              if ($result) {
              	  while ($data = $result->fetch_assoc()) {
		...
		<div class="col-sm-4">
			...
			<div class="single-products">				
				<div class="productinfo text-center">
					...
					<h2>$<?php echo $data['price'];?></h2>
					<p><?php echo $data['name'];?></p>
					<a href="detail?seeProductId=<?php echo $data['id'];?>" class="btn btn-default add-to-cart"><i class="fa fa-shopping-cart"></i>See Details</a>
				</div>					
			  </div>		
			</div>
		</div>
